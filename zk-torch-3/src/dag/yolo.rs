...
pub fn yolov11n(
    ...
    move |g, inputs| {
        assert_eq!(inputs.len(), 1, "YOLOv11 expects 1 input");
        let mut wi = 0;

        // Helper to load N weight pairs
        let mut load_n = |g: &mut DagBuilder, n: usize| -> Vec<(EdgeId, EdgeId)> {
        ...
        let w0 = load_n(g, 1);
        let h = cbs(g, inputs[0], w0[0].0, w0[0].1, (3, 3), (2, 2), (1, 1));  // [16, 320, 320]
...
pub fn generate_yolo_weight(
    ...
    if is_depthwise {
        ...
        let mut data = vec![goldilocks_cuda::GoldilocksField(0); size];
        for ch in 0..c {
            for ki in 0..kh {
                for kj in 0..kw {
                    let idx = kj + ki * kw_pad + 0 * kw_pad * kh_pad + ch * 1 * kw_pad * kh_pad;
                    data[idx] = goldilocks_cuda::GoldilocksField((rng.gen::<u32>() % 100) as u64);
                }
            }
        }
        let w = Witness::new(vec![c, 1, kh, kw], data, DataType::Uint, 0, Role::Constant);
        ...
        let bias_vec: Vec<goldilocks_cuda::GoldilocksField> = (0..c)
            .map(|_| goldilocks_cuda::GoldilocksField((rng.gen::<u32>() % 50) as u64))
            .collect();
        let mut bdata = vec![goldilocks_cuda::GoldilocksField(0); bsize];
        for ci in 0..c {
            for hi in 0..h_out {
        ...
        let b = Witness::new(vec![c, h_out, w_out], bdata, DataType::Uint, 0, Role::Constant);
        (w, b)
    } else {
        ...
        let w = Witness::new(vec![c_out, c_in, kh, kw], data, DataType::Uint, 0, Role::Constant);
        ...
        let w_pad = w_out.next_power_of_two();
        let bsize = c_pad2 * h_pad * w_pad;
        let bias_vec: Vec<goldilocks_cuda::GoldilocksField> = (0..c_out)
            .map(|_| goldilocks_cuda::GoldilocksField((rng.gen::<u32>() % 50) as u64))
            .collect();
        let mut bdata = vec![goldilocks_cuda::GoldilocksField(0); bsize];
        for ci in 0..c_out {
        ...
        let b = Witness::new(vec![c_out, h_out, w_out], bdata, DataType::Uint, 0, Role::Constant);
        (w, b)
    }
}
