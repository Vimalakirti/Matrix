//! One-shot autoregressive proving for LLaMA 3.1 8B.
//!
//! Pipeline: embedding(W_E) → llama3_8b body (transformer + final RMS + lm_head) → argmax check.
//! `llama3_8b` already produces `[1, seq_len, vocab_size]` logits, so we change_shape to
//! `[seq_len, vocab_size]` and pass straight into argmax_check.
//!
//! LLaMA 3 uses RoPE inside attention — no absolute positional embeddings.
//!
...
fn generate_llama3_weights(num_layers: usize, vocab_size: usize) -> (
    ...
    for _ in 0..num_layers {
        ...
        attn_q_w_vec.push(Witness::new(vec![HIDDEN_DIM, HIDDEN_DIM], generate_random_field_vec(padded_size(&[HIDDEN_DIM, HIDDEN_DIM])), DataType::Float, *SF_LOG, Role::Constant));
        attn_k_w_vec.push(Witness::new(vec![HIDDEN_DIM, KV_DIM], generate_random_field_vec(padded_size(&[HIDDEN_DIM, KV_DIM])), DataType::Float, *SF_LOG, Role::Constant));
        attn_v_w_vec.push(Witness::new(vec![HIDDEN_DIM, KV_DIM], generate_random_field_vec(padded_size(&[HIDDEN_DIM, KV_DIM])), DataType::Float, *SF_LOG, Role::Constant));
        attn_o_w_vec.push(Witness::new(vec![HIDDEN_DIM, HIDDEN_DIM], generate_random_field_vec(padded_size(&[HIDDEN_DIM, HIDDEN_DIM])), DataType::Float, *SF_LOG, Role::Constant));
        proj_norm_w_vec.push(Witness::new(vec![HIDDEN_DIM], generate_random_field_vec(HIDDEN_DIM), DataType::Float, *SF_LOG, Role::Constant));
        proj_1_w_vec.push(Witness::new(vec![HIDDEN_DIM, FFN_DIM], generate_random_field_vec(padded_size(&[HIDDEN_DIM, FFN_DIM])), DataType::Float, *SF_LOG, Role::Constant));
        proj_2_w_vec.push(Witness::new(vec![HIDDEN_DIM, FFN_DIM], generate_random_field_vec(padded_size(&[HIDDEN_DIM, FFN_DIM])), DataType::Float, *SF_LOG, Role::Constant));
        proj_3_w_vec.push(Witness::new(vec![FFN_DIM, HIDDEN_DIM], generate_random_field_vec(padded_size(&[FFN_DIM, HIDDEN_DIM])), DataType::Float, *SF_LOG, Role::Constant));
    }
...
fn main() {
    ...
    let num_layers: usize = std::env::var("NUM_LAYERS").ok().and_then(|s| s.parse().ok()).unwrap_or(32);
    ...
    let (
        ...
        proj_norm_w_vec, proj_1_w_vec, proj_2_w_vec, proj_3_w_vec,
        layer_norm_w, logits_w,
    ) = generate_llama3_weights(num_layers, vocab_size);
    ...
    // llama3_8b: outputs `[1, seq_len, vocab_size]` logits (final RMS + matmul baked in).
    ...
    if num_partitions > 1 {
        ...
        let mut transcript = Transcript::new(b"zkml-oneshot-llama3");
        let t4 = Instant::now();
        ...
        let mut verify_transcript = Transcript::new(b"zkml-oneshot-llama3");
        let t5 = Instant::now();
