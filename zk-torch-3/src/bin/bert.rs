...
use zk_torch_3::dag::bert::bert_large;
...
fn generate_bert_weights(num_layers: usize) -> (
    ...
    for _i in 0..num_layers {
        attn_norm_w_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_q_w_vec.push(Witness::new(vec![1024, 1024], generate_random_field_vec(1024 * 1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_k_w_vec.push(Witness::new(vec![1024, 1024], generate_random_field_vec(1024 * 1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_v_w_vec.push(Witness::new(vec![1024, 1024], generate_random_field_vec(1024 * 1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_o_w_vec.push(Witness::new(vec![1024, 1024], generate_random_field_vec(1024 * 1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_norm_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_q_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_k_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_v_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        attn_o_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        proj_norm_w_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        proj_1_w_vec.push(Witness::new(vec![1024, 4096], generate_random_field_vec(1024 * 4096), DataType::Float, *SF_LOG, Role::Constant));
        proj_2_w_vec.push(Witness::new(vec![4096, 1024], generate_random_field_vec(4096 * 1024), DataType::Float, *SF_LOG, Role::Constant));
        proj_norm_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
        proj_1_b_vec.push(Witness::new(vec![4096], generate_random_field_vec(4096), DataType::Float, *SF_LOG, Role::Constant));
        proj_2_b_vec.push(Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant));
    }

    let layer_norm_w = Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant);
    let layer_norm_b = Witness::new(vec![1024], generate_random_field_vec(1024), DataType::Float, *SF_LOG, Role::Constant);
    let matmul_w = Witness::new(vec![1024, 2], generate_random_field_vec(1024 * 2), DataType::Float, *SF_LOG, Role::Constant);
    let matmul_b = Witness::new(vec![2], generate_random_field_vec(2), DataType::Float, *SF_LOG, Role::Constant);
...
fn main() {
    ...
    println!("=== BERT-Large on Goldilocks ({} layers, seq_len={}) ===", num_layers, seq_len);
    ...
    let (
        ...
        proj_norm_b_vec, proj_1_b_vec, proj_2_b_vec,
        layer_norm_w, layer_norm_b, matmul_w, matmul_b,
    ) = generate_bert_weights(num_layers);
    ...
    let _output = g.pipe(
        ...
        bert_large(
            ...
            proj_norm_w_vec, proj_1_w_vec, proj_2_w_vec,
            proj_norm_b_vec, proj_1_b_vec, proj_2_b_vec,
            layer_norm_w, layer_norm_b, num_heads, head_dim, seq_len, matmul_w, matmul_b,
        ),
    );
    ...
    let padded_size = input_size.next_power_of_two();
    let input = Witness::new(
        vec![1, seq_len, hidden_dim],
        generate_random_field_vec(padded_size),
    ...
    if num_partitions > 1 {
        ...
        let mut transcript = Transcript::new(b"zkml-bert");
        let t4 = Instant::now();
        ...
        let mut verify_transcript = Transcript::new(b"zkml-bert");
        let t5 = Instant::now();
