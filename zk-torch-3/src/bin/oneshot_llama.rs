//! One-shot autoregressive proving for LLaMA-2-7B.
//!
//! Pipeline: embedding(W_E) → RoPE-precompute → llama_block × num_layers → final RMS norm
//!         → lm_head(logits_w) → change_shape → argmax check.
//!
//! We re-implement the body inline (instead of calling `llama_2_7b`) because that helper
//! reshapes the final logits to `[1, vocab_size]`, dropping the seq dimension. Oneshot
//! needs the full `[seq_len, vocab_size]` logits.
//!
//! LLaMA uses RoPE inside attention — no absolute positional embeddings.
//!
...
use zk_torch_3::dag::llama::{
    llama_block, llama_rms_norm, rope_cos_mat_llama3, rope_sin_mat_llama3,
};
...
fn generate_llama_weights(num_layers: usize, vocab_size: usize) -> (
...
fn main() {
    ...
    let (
        ...
        proj_norm_w_vec, proj_1_w_vec, proj_2_w_vec, proj_3_w_vec,
        layer_norm_w, logits_w,
    ) = generate_llama_weights(num_layers, vocab_size);
    ...
    let mut x = g.change_shape(h0, vec![1, seq_len, HIDDEN_DIM]);

    // Precompute RoPE cos/sin (shared across layers; matches llama_2_7b body).
    let theta = 10000.0;
    ...
    let cos_param = g.param(Witness::new(
        vec![seq_len, HEAD_DIM], cos_data,
        DataType::Float, *SF_LOG, Role::Constant,
    ));
    let sin_param = g.param(Witness::new(
        vec![seq_len, HEAD_DIM], sin_data,
        DataType::Float, *SF_LOG, Role::Constant,
    ));
    ...
    // Final RMS norm
    let final_norm_w = g.param(layer_norm_w);
    let h_out = g.pipe(&[x], llama_rms_norm(final_norm_w))[0];

    // LM head: logits_w shape (hidden_dim, vocab_size), einsum "bsi,ij->bsj".
    let logits_w_edge = g.param(logits_w);
    let logits = g.lm_head(h_out, logits_w_edge, seq_len, vocab_size);
    ...
    if num_partitions > 1 {
        ...
        let mut transcript = Transcript::new(b"zkml-oneshot-llama");
        let t4 = Instant::now();
        ...
        let mut verify_transcript = Transcript::new(b"zkml-oneshot-llama");
        let t5 = Instant::now();
