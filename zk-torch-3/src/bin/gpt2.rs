...
fn main() {
    ...
    println!("=== GPT-2 Small on Goldilocks ({} layers, seq_len={}) ===", num_layers, seq_len);
    ...
    let padded_size: usize = [1usize, seq_len, hidden_dim].iter().map(|&s| s.next_power_of_two()).product();
    ...
    if num_partitions > 1 {
        ...
        let mut transcript = Transcript::new(b"zkml-gpt2");
        let t4 = Instant::now();
        ...
        let mut verify_transcript = Transcript::new(b"zkml-gpt2");
        let t5 = Instant::now();
