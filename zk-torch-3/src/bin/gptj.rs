...
fn generate_gptj_weights(num_layers: usize) -> (
    ...
    for _i in 0..num_layers {
        attn_norm_w_vec.push(Witness::new(vec![4096], generate_random_field_vec(4096), DataType::Float, *SF_LOG, Role::Constant));
        attn_q_w_vec.push(Witness::new(vec![4096, 4096], generate_random_field_vec(4096 * 4096), DataType::Float, *SF_LOG, Role::Constant));
        attn_k_w_vec.push(Witness::new(vec![4096, 4096], generate_random_field_vec(4096 * 4096), DataType::Float, *SF_LOG, Role::Constant));
        attn_v_w_vec.push(Witness::new(vec![4096, 4096], generate_random_field_vec(4096 * 4096), DataType::Float, *SF_LOG, Role::Constant));
        attn_o_w_vec.push(Witness::new(vec![4096, 4096], generate_random_field_vec(4096 * 4096), DataType::Float, *SF_LOG, Role::Constant));
        attn_norm_b_vec.push(Witness::new(vec![4096], generate_random_field_vec(4096), DataType::Float, *SF_LOG, Role::Constant));
        proj_1_w_vec.push(Witness::new(vec![4096, 16384], generate_random_field_vec(4096 * 16384), DataType::Float, *SF_LOG, Role::Constant));
        proj_2_w_vec.push(Witness::new(vec![16384, 4096], generate_random_field_vec(16384 * 4096), DataType::Float, *SF_LOG, Role::Constant));
        proj_1_b_vec.push(Witness::new(vec![16384], generate_random_field_vec(16384), DataType::Float, *SF_LOG, Role::Constant));
        proj_2_b_vec.push(Witness::new(vec![4096], generate_random_field_vec(4096), DataType::Float, *SF_LOG, Role::Constant));
    }

    let layer_norm_w = Witness::new(vec![4096], generate_random_field_vec(4096), DataType::Float, *SF_LOG, Role::Constant);
    let layer_norm_b = Witness::new(vec![4096], generate_random_field_vec(4096), DataType::Float, *SF_LOG, Role::Constant);
    let matmul_w = Witness::new(vec![4096, 50400], generate_random_field_vec(4096 * 65536), DataType::Float, *SF_LOG, Role::Constant);
    let matmul_b = Witness::new(vec![50400], generate_random_field_vec(65536), DataType::Float, *SF_LOG, Role::Constant);

    (
        attn_norm_w_vec, attn_q_w_vec, attn_k_w_vec, attn_v_w_vec, attn_o_w_vec,
        attn_norm_b_vec,
        proj_1_w_vec, proj_2_w_vec,
        proj_1_b_vec, proj_2_b_vec,
...
fn main() {
    ...
    println!("=== GPT-J 6B on Goldilocks ({} layers, seq_len={}) ===", num_layers, seq_len);
    ...
    let (
        ...
        proj_1_b_vec, proj_2_b_vec,
        layer_norm_w, layer_norm_b, matmul_w, matmul_b,
    ) = generate_gptj_weights(num_layers);
    ...
    if num_partitions > 1 {
        ...
        let mut transcript = Transcript::new(b"zkml-gptj");
        let t4 = Instant::now();
        ...
        let mut verify_transcript = Transcript::new(b"zkml-gptj");
        let t5 = Instant::now();
