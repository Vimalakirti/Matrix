...
pub fn partition_dag(dag: &Dag, boundary_edges: &[EdgeId]) -> Vec<PartitionDesc> {
    ...
    for k in 0..num_partitions {
        ...
        // Classify boundary edges. A "boundary" is ANY edge that crosses
        // between partitions and HAS a producer — not just the designated
        // cut edges (`boundary_set`). Branchy graphs (e.g. Whisper's decoder
        // cross-attention reading the encoder output, or a range/ScaleDown
        // node whose output is consumed in another partition) create
        // cross-partition edges beyond the single designated layer boundary;
        // every one needs a seeded boundary claim so the producer's partition
        // proves the producer (otherwise it is orphaned → its input edge is
        // never claimed → prove_range panics). For clean sequential chains
        // (GPT-2/Llama-2) the only cross-partition edges ARE the designated
        // boundaries, so this is identical to the old behavior.
        //   - boundary_input: an input edge WITH a producer (necessarily in
        //     another partition, since input_edges excludes produced-here).
        //     Weights / DAG inputs (no producer) are committed separately, not
        //     boundary claims.
        //   - boundary_output: an output edge consumed by another partition
        //     (non-empty consumers ⇒ not a pure DAG output).
        let _ = &boundary_set; // designated cuts drive node assignment above
        partitions[k].boundary_input_edges = partitions[k]
            .input_edges
            .iter()
            .filter(|&&e| dag.producers[e].is_some())
            .copied()
            .collect();
        partitions[k].boundary_output_edges = partitions[k]
            .output_edges
            .iter()
            .filter(|&&e| !dag.consumers[e].is_empty())
            .copied()
            .collect();
    }
...
/// All cross-partition edges (produced in one partition, consumed in another)
/// — the full set of edges that need a seeded boundary claim. Sorted, deduped.
/// Union of every partition's `boundary_output_edges`.
pub fn cross_partition_edges(partitions: &[PartitionDesc]) -> Vec<EdgeId> {
    let mut s: HashSet<EdgeId> = HashSet::new();
    for p in partitions {
        s.extend(p.boundary_output_edges.iter().copied());
    }
    let mut v: Vec<EdgeId> = s.into_iter().collect();
    v.sort();
    v
}
