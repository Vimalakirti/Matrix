...
/// Fixed-point rescale after a conv (weight sf>0): 2^(2·sf) → 2^sf. No-op for
/// sf=0 (integer) weights. Mirrors resnet/vgg/yolo.
...
pub fn pointpillar(
    ...
    move |g, inputs| {
        assert!(inputs.len() >= 2 && inputs.len() % 2 == 0,
            "PointPillar expects 2 inputs (pillars + coords) per batch sample");
        ...
        // ---- Phase 1: create the SHARED (weight, bias) param edges ONCE.
        // Each batch sample reuses these, committing the weights exactly once
        // (and, streaming, deferring + amortizing those Constant edges). ----
        let params: Vec<(EdgeId, EdgeId)> = all_weights
            .iter()
            .map(|(w, b)| (g.param(w.clone()), g.param(b.clone())))
            .collect();

        // ---- Phase 2: build the body once per batch sample (shared edges). ----
        let mut outs = Vec::with_capacity(inputs.len() / 2 * 3);
        for pair in inputs.chunks(2) {
            outs.extend(pointpillar_body(
                g, pair[0], pair[1], &params, ny, nx, n_pillars, max_points));
        }
        outs
    }
}

/// One PointPillar forward pass for a single sample (`pillar_input`, `coords`),
/// reusing the shared `params`. `take(n)` slices the pre-built param edges.
...
fn pointpillar_body(
    ...
    {
        let mut wi = 0usize;
        let mut take = |n: usize| -> Vec<(EdgeId, EdgeId)> {
            let s = params[wi..wi + n].to_vec();
            wi += n;
            s
        };
        ...
        let __vfe_sf = g.init_values[vfe_w[0].0].as_ref().unwrap().sf;
        let h = g.einsum("abc,cd->abd".to_string(), vec![pillar_input, vfe_w[0].0], __vfe_sf > 0)[0];
