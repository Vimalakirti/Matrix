//! Llama-3.1-8B streaming-inference bench. Mirror of
//! `bench_streaming_llama2` on the `llama3_8b` DAG (GQA, RoPE theta=500000,
//! SwiGLU, un-tied lm_head), so the two rows measure the SAME thing: each
//! streamed proof is one forward pass over SEQ_LEN tokens, and the weights are
//! Role::Constant, deferred across the stream and opened once at finalize.
//!
//! This is deliberately NOT `bench_streaming_oneshot_llama3`, which already
//! existed. That one streams full T-token AR GENERATIONS -- a different
//! workload, not comparable to the GPT-2 / BERT / Llama-2 rows in the same
//! table.
//!
//! Env: NUM_LAYERS(1) SEQ_LEN(1) N_PROOFS(5) MAX_NUM_VARS(29) HIDDEN_DIM(4096)
//! NUM_HEADS(32) NUM_KV_HEADS(8) HEAD_DIM(128) FFN_DIM(14336)
//! VOCAB_SIZE(128256) NUM_PARTITIONS(1) ZK4_B(21) ZK4_BASE(2).
//!
//! `llama3_8b` does not shard the logits head or the FFN projections, so
//! MAX_NUM_VARS defaults to 29 to fit the largest committed edge at full
//! config; override the model dims for smaller runs.
...
/// Per-iteration small-varied input. RMSNorm does not subtract the mean, so a
/// near-constant input around 1024 keeps mean(x^2) ~ sf and the reciprocity
/// gate well-defined with rounding error 0. Varying it per iteration produces
/// distinct proofs, so the reducer exercises the real fold rather than the
/// degenerate identity case.
fn small_varied_input(size: usize, iter_offset: u64) -> Vec<AlmostGoldilocksField> {
    (0..size)
        .map(|i| {
            let offset = (i as u64 + iter_offset).wrapping_mul(7) % 4;
            AlmostGoldilocksField(1024 + offset)
        })
        .collect()
}
...
fn main() {
    ...
    let max_num_vars: usize = std::env::var("MAX_NUM_VARS").ok().and_then(|s| s.parse().ok()).unwrap_or(29);
    let hidden_dim: usize = std::env::var("HIDDEN_DIM").ok().and_then(|s| s.parse().ok()).unwrap_or(4096);
    ...
    let vocab_size: usize = std::env::var("VOCAB_SIZE").ok().and_then(|s| s.parse().ok()).unwrap_or(128256);
    ...
    println!("=== Llama-3.1-8B streaming-inference bench ===");
    println!("num_layers={} seq_len={} N_PROOFS={} max_num_vars={}", num_layers, seq_len, n_proofs, max_num_vars);
    println!("hidden_dim={} ffn_dim={} heads={} kv_heads={} head_dim={} vocab={}",
    ...
    // ---- Streaming loop ----
    let label = b"zkml-llama3-streaming";
    ...
    for i in 0..n_proofs {
        ...
        let input = Witness::new(
            ...
            small_varied_input(pad, i as u64),
