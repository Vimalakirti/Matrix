//! Streaming one-shot AR Llama-3.1 (GQA, RoPE θ=500000, SwiGLU, un-tied
...
//! Mirror of bench_streaming_oneshot_llama2 with the llama3_8b_hidden body.
//!
//! Run with `bench_config.yaml` as args[1]. Env: NUM_LAYERS(1) SEQ_LEN(4)
//! VOCAB_SIZE(256) N_PROOFS(5) NUM_HEADS(8) NUM_KV_HEADS(8) HEAD_DIM(64)
...
fn main() {
    ...
    println!("=== Streaming one-shot AR Llama-3.1 ===");
    println!("num_layers={} seq_len={} vocab={} N_PROOFS={} hidden={} heads={} kv_heads={} head_dim={} ffn={} partitions={}",
             num_layers, seq_len, vocab_size, n_proofs, hidden_dim, num_heads, num_kv_heads, head_dim, ffn_dim, num_partitions);
    ...
    let h_out = g.pipe(&[h_in], llama3_8b_hidden(
        ...
        // These bins are not FFN-sharded; a single-element inner vec is the
        // un-sharded form llama_mlp already handled.
        weights.6.into_iter().map(|w| vec![w]).collect(),
        weights.7.into_iter().map(|w| vec![w]).collect(),
        weights.8.into_iter().map(|w| vec![w]).collect(),
        weights.9,
    ...
    let label = b"zkml-oneshot-llama3-streaming";
