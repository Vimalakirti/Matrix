//! Llama-2 streaming-inference bench. Mirror of
//! `bench_streaming_gpt2` for the Llama-2-7B DAG. See that file's
//! docstring for the streaming pattern and per-iteration semantics.
//!
//! Env knobs:
//!   - `NUM_LAYERS` (default 1), `SEQ_LEN` (default 1)
//!   - `NUM_HEADS`, `HEAD_DIM`, `FFN_DIM`, `VOCAB` — model shape
//!     (defaults to Llama-2-7B); override for smaller smoke tests.
//!   - `LOGITS_SHARDS` (default 32), `FFN_SHARDS` (default 16).
...
/// Per-iteration small-varied input. Llama's RMSNorm doesn't subtract
/// the mean (unlike LayerNorm), so a near-constant input ≈ 1024 keeps
/// mean(x²) ≈ sf and the reciprocity check well-defined with rounding
/// error 0. Adding small offsets per iteration produces distinct
/// proofs so the reducer exercises the K=2 fold rather than the
/// degenerate identity case.
fn small_varied_input(size: usize, iter_offset: u64) -> Vec<AlmostGoldilocksField> {
    // 1024 + small variation around 1.0; tolerance=2 in RMSNorm's
    // reciprocity gate accepts the resulting rounding error.
    (0..size)
        .map(|i| {
...
fn gen_llama2_weights(
    ...
    for _ in 0..num_layers {
        ...
        proj_norm_w.push(Witness::new(vec![hidden_dim], zero_field_vec(hd_pad), DataType::Float, *SF_LOG, Role::Constant));
        let p1: Vec<Witness> = (0..ffn_shards).map(|_| {
            Witness::new(vec![hidden_dim, shard_ffn],
                zero_field_vec(hd_pad * shard_ffn_pad),
        ...
        let p3: Vec<Witness> = (0..ffn_shards).map(|_| {
            Witness::new(vec![shard_ffn, hidden_dim],
                zero_field_vec(shard_ffn_pad * hd_pad),
                DataType::Float, *SF_LOG, Role::Constant)
        }).collect();
        proj_1_w.push(p1);
        proj_2_w.push(p2);
    ...
    let layer_norm_w = Witness::new(vec![hidden_dim], zero_field_vec(hd_pad), DataType::Float, *SF_LOG, Role::Constant);
    let logits_w_shards: Vec<Witness> = (0..logits_shards).map(|_| {
        Witness::new(
            vec![hidden_dim, shard_vocab],
            zero_field_vec(hd_pad * shard_vocab_pad),
            DataType::Float, *SF_LOG, Role::Constant,
        )
    }).collect();
    (
...
fn main() {
    ...
    let vocab: usize = std::env::var("VOCAB").ok().and_then(|s| s.parse().ok()).unwrap_or(32000);
    ...
    println!("=== Llama-2 streaming-inference bench ===");
    println!("num_layers={} seq_len={} N_PROOFS={} max_num_vars={}", num_layers, seq_len, n_proofs, max_num_vars);
    println!("hidden_dim={} ffn_dim={} vocab={} logits_shards={} ffn_shards={}",
             hidden_dim, ffn_dim, vocab, logits_shards, ffn_shards);
    ...
    // ---- Streaming loop ----
    let label = b"zkml-llama2-streaming";
