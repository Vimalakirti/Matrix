//! Streaming one-shot AR Llama-2: one-shot AR proving (embedding → llama
//! transformer (RoPE, SwiGLU) → un-tied lm_head → argmax_check) composed
//! with the cross-proof streaming accumulator. Each streamed proof is a full
//! T-token generation; across the N generations the WEIGHTS (W_E, transformer,
//! W_LM) are Role::Constant (deferred → amortized into one finalize opening)
//! and the per-generation SELECTORS (embedding + argmax) are Role::Input
//! (committed/opened per-proof, never deferred). Mirror of
//! bench_streaming_oneshot_gpt2 with the Llama body (no positional add — RoPE;
//! un-tied lm_head). RMSNorm → constant embedding is fine.
//!
...
//! FFN_SHARDS(4) MAX_NUM_VARS(22) NUM_PARTITIONS(1).
...
/// RMSNorm-friendly embedding (constant 1024; no mean-subtract).
...
fn gen_llama_weights(num_layers: usize, hd: usize, ffn: usize, ffn_shards: usize) -> (
    ...
    (anw, aqw, akw, avw, aow, pnw, p1, p2, p3, c(vec![hd], hdp))
}
...
fn main() {
    ...
    println!("=== Streaming one-shot AR Llama-2 ===");
    ...
    let weights = gen_llama_weights(num_layers, hidden_dim, ffn_dim, ffn_shards);
    ...
    let (logits_edges, argmax_sels, shard_ranges): (Vec<EdgeId>, Vec<EdgeId>, Vec<(usize, usize)>) =
        if vocab_shards > 1 {
            ...
            let w_lm_w = Witness::new(vec![vocab_size, hidden_dim],
                vec![AlmostGoldilocksField(1024); vocab_size.next_power_of_two() * hidden_dim.next_power_of_two()],
                DataType::Float, *SF_LOG, Role::Constant);
    ...
    let label = b"zkml-oneshot-llama2-streaming";
